...
- These marks are fullytaken care of as long **Uniform Interface** is maintained!
...
- When client asks server `hmtl` its returned from `server`. In REST context `<Response>` is send back intead of `<html>`
